  ...
  <div class="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-6 stagger">
    ...
    <div class="surface spot-card p-8 flex items-center gap-6 lg:col-span-1 rounded-3xl">
      ...
      <div>
        ...
        <p class="text-xs text-[var(--muted)] mt-1"><?= (int)$profile['tasks_completed'] ?> studi kasus &middot; lintas semua divisi</p>
      </div>
    </div>
      ...
      <?php else: ?>
        <div class="space-y-4 divide-y divide-[var(--border-light)]">
          ...
          <div class="<?= $i > 0 ? 'pt-4' : '' ?>">
            ...
            <p class="text-[11px] text-[var(--muted-light)] mt-2"><?= (int)$track['tasks_completed'] ?> studi kasus &middot; skor pemahaman rata-rata <?= (int)$track['comprehension_avg'] ?>/100</p>
          </div>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
