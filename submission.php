...
<section class="max-w-4xl mx-auto px-6 py-10">
  ...
  <div class="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6 stagger">
    <div class="surface spot-card p-8 flex items-center gap-6 lg:col-span-1">
      <div class="relative w-24 h-24 shrink-0">
        ...
        <div class="absolute inset-0 grid place-items-center">
          <span class="num text-2xl font-extrabold text-[var(--ink)]"><?= (int)$review['overall_score'] ?></span>
        </div>
      </div>
      <div>
    ...
    <div class="lg:col-span-2 surface p-8 grid grid-cols-3 gap-4">
      <div class="text-center p-4 rounded-2xl hover:bg-[var(--accent-50)] transition-colors">
        <div class="w-10 h-10 rounded-xl mx-auto mb-3 flex items-center justify-center bg-[var(--accent-50)] text-[var(--accent)] border border-[var(--accent-100)]">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>
        </div>
        <p class="num text-2xl font-extrabold <?= score_color_class((int)$review['clean_code_score']) ?>"><?= (int)$review['clean_code_score'] ?></p>
      ...
      <div class="text-center p-4 rounded-2xl hover:bg-[var(--accent-50)] transition-colors">
        ...
        <p class="num text-2xl font-extrabold <?= score_color_class((int)$review['security_score']) ?>"><?= (int)$review['security_score'] ?></p>
      ...
      <div class="text-center p-4 rounded-2xl hover:bg-[var(--accent-50)] transition-colors">
        ...
        <p class="num text-2xl font-extrabold <?= score_color_class((int)$review['efficiency_score']) ?>"><?= (int)$review['efficiency_score'] ?></p>
  ...
  <div class="mt-10">
    <h2 class="text-lg font-bold mb-5 flex items-center gap-2">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--ink)" stroke-width="2"><path d="M9 12l2 2 4-4m6 2a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/></svg>
    ...
    <div class="space-y-3 stagger">
      <?php foreach ($findings as $f):
        $sevConfig = [
            ...
            'info' => ['class' => 'badge-accent', 'label' => 'Info', 'icon' => '<circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/>'],
        ];
        $sev = $sevConfig[$f['severity']] ?? $sevConfig['info'];
        $source = $f['source'] ?? 'ai-judged';
      ?>
      <div class="surface p-5 flex items-start gap-4 transition-all duration-200 hover:border-[var(--accent-200)] hover:shadow-md">
